...
namespace ArtifactDissimilarity
{
    public class UnisonBackup
    {
        ...
        public static void OnArtifactDisable()
        {
            ...
            On.RoR2.PickupPickerController.SetOptionsServer -= PickupPickerController_SetOptionsServer;
            On.RoR2.ChestBehavior.ItemDrop -= Unison_Simplified.ChestBehavior_ItemDrop;
            //
        ...
        {
            //Ideally I wouldn't need to mess with the Runs available item drops
            //But multiple mods are stupid and use that directly instead of using Drop Tables
            //Scavs are about the only thing left that still uses it in the vanilla game
            On.RoR2.PickupDropTable.GenerateDrop += PickupDropTable_GenerateDrop;
            On.RoR2.PickupPickerController.SetOptionsServer += PickupPickerController_SetOptionsServer;
